...
import 'package:http_requests/widgets/example_model.dart';
...
class _ExampleState extends State<Example> {
  final model = ExampleInheritWidgetModel();
...
class _CreateButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ElevatedButton(
        onPressed: () =>
            ExampleModelProvider.read(context)?.model.createPost(),
...
class _PostsWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
        itemCount: ExampleModelProvider.read(context)!.model.posts.length ?? 0,
